...
from bw.cfg import BOOK_DIR
from bw.utils.file import load_epub, analyze_book
...
def find_book_file(person_name: str, book_title: str) -> Path | None:
    ...
    for epub_file in Path(BOOK_DIR).glob("*.epub"):
            ...
            try:
                ...
                if 0 <= person_idx < len(persons):
                    ...
                    if book_file:
                        ...
                        profile = analyze_book(book, person.get("birth_year"))  # TODO: currently only rely on lm api cache, should query db (lazy load)
                        self.send_json({
                            "title": book.title,
                            "author": book.author,
                            "chapters": [
