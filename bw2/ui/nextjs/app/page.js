...
function Reader({ person, personIndex, desiredAge, onClose, onAddToLibrary }) {
    ...
    useEffect(() => {
        if (!book || !contentRef.current || !measureRef.current) return
        ...
        measureRef.current.innerHTML = `<h3 style="font-size: 20px; margin-bottom: 20px; margin-top: 0;">${chapter.title}</h3>`
        const titleHeight = measureRef.current.scrollHeight

        paragraphs.forEach((paragraph, idx) => {
            const testContent = currentPageParagraphs.length === 0
                ? `<h3 style="font-size: 20px; margin-bottom: 20px; margin-top: 0;">${chapter.title}</h3><p style="margin-bottom: 1.2em; text-align: justify;">${paragraph}</p>`
                : currentPageParagraphs.map(p => `<p style="margin-bottom: 1.2em; text-align: justify;">${p}</p>`).join('') + `<p style="margin-bottom: 1.2em; text-align: justify;">${paragraph}</p>`

            measureRef.current.innerHTML = testContent
            const testHeight = measureRef.current.scrollHeight

            if (testHeight > availableHeight && currentPageParagraphs.length > 0) {
                paginatedPages.push([...currentPageParagraphs])
    ...
    return (
        <div style={{ position: 'absolute', inset: 0, background: '#faf8f5', display: 'flex', flexDirection: 'column', zIndex: 100 }}>
            ...
            <div
                ref={contentRef}
                style={{ flex: 1, padding: '30px 25px', overflow: 'hidden', fontSize: 16, lineHeight: 1.8, color: '#333', position: 'relative' }}
                onClick={handleClick}
                onTouchStart={handleTouchStart}
                onTouchEnd={handleTouchEnd}
            >
                {currentPage === 0 && <h3 style={{ fontSize: 20, marginBottom: 20 }}>{chapter.title}</h3>}
                {pageContent.map((p, i) => (
            ...
            <div style={{ textAlign: 'center', padding: 5, fontSize: 12, color: '#666' }}>
                Chapter {currentChapter + 1}/{book.chapters.length} • Page {currentPage + 1}/{totalPages || 1}
            </div>
